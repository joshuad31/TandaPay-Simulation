...
class Simulator:
    ...
    def __init__(self, ui=True, _matrix=None, edge=False):
        ...
        if self.ui is True:
            ...
            for run, vector in enumerate(self._matrix):
                for _row in sh_user['A2:N200']:
                    for _cell in _row:
                        _cell.value = None

                for _row in sh_system['C2:U37']:
                    for _cell in _row:
                        _cell.value = None
                ...
                wb_system.save(path_system)
                self.run = run + 1
                print(f'____ Run Number {self.run} ____')
                self.ev = vector[:9]
                self.ev.append(0)
                self.pv = vector[9:]
                self.start_ev = self.ev.copy()
                self.start_pv = self.pv.copy()
                self.start_action(vector=True)
    ...
    def show_btns(self):
        start_btn = tk.Button(self.app, text="Start", width=10, height=1, bg='green', fg='white',
                              command=self.start_action)
        start_btn.grid(row=17, column=1, padx=30, sticky='w')

        stop_btn = tk.Button(self.app, text="Stop", width=10, height=1, bg='red', fg='white', command=self.stopAction)
        stop_btn.grid(row=17, column=2, padx=30, sticky='w')

        clear_btn = tk.Button(self.app, text="Clear", width=10, height=1, command=self.clearAction)
        clear_btn.grid(row=17, column=3, padx=30, sticky='w')

        close_btn = tk.Button(self.app, text="Close", width=10, height=1, command=self.closeAction)
        close_btn.grid(row=17, column=4, padx=30, sticky='w')
    ...
    def start_action(self, vector=None):
        ...
        while start_iter < 10:
            ...
            if counter == 1:
                if not vector:
                    ...
                    print('Assigning vector values to EV and PV arrays')
            ...
            if current_period_list[get_period] == 'Period Data 1':
                ...
                wb_user.save(path_user)
                print(
                    'group of four members: {}, group of five members: {}, group of six members: {}, group of seven members: {}, Total group: {})'.format(
                        step14, step3, step7, step11, step14 * 4 + step3 * 5 + step7 * 6 + step11 * 7))
                ...
                try:
                    if not self._matrix:
                        ...
                        if not self.edge:
                            ...
                            matrix_var_sh_cols = [ev4_cell, pv4_cell, pv5_cell, ev3_cell, self.ev1_cell, ev5_cell,
                                                  ev6_cell, ev7_cell]
...
def main():
    # simulator = Simulator(ui=False, _matrix=[[53, 1000, 0, 35, 64, 100, 4, 0, 99, 40, 2, 70, 20, 80, 5]],edge=True)
    # simulator = Simulator(ui=False, _matrix=[[60, 1000, 40, 15, 10, 65, 3, 3, 0.33333, 20, 4, 70, 8, 77, 3]])
    # simulator = Simulator(ui=False, _matrix=[
    #  [53, 1000, 0, 13, 40, 100, 4, 0, 99, 10, 2, 70, 20, 80, 5],
    #  [53, 1000, 0, 13, 40, 100, 4, 3, 99, 10, 2, 70, 20, 80, 5],
    #  [53, 1000, 100, 10, 40, 100, 4, 3, 99, 10, 2, 70, 20, 80, 5],
    #  [53, 1000, 100, 30, 0, 100, 4, 0, 99, 10, 2, 70, 20, 80, 5], # defs multiple of 4, PV1=40
    #  [70, 1000, 0, 15, 100, 100, 2, 0, 99, 10, 2, 70, 20, 80, 5],
    #  [53, 1000, 0, 35, 64, 100, 4, 0, 99, 40, 2, 70, 20, 80, 5],
    # [80, 1000, 65, 27, 23, 99, 2, 3, 0.3333, 15, 3, 70, 15, 80, 5],
    # [72, 1000, 65, 25, 15, 99, 2, 0, 0.5, 10, 2, 80, 20, 90, 5],
    # [70, 1000, 75, 22, 18, 80, 2, 0, 0.5, 10, 2, 70, 20, 80, 5],
    # [61,1000,0,35,64,100,4,0,0.99,40,2,70,20,80,5],
    # [61,1000,0,35,0,100,4,0,0.01,40,2,70,20,80,5]

    # ],edge=True)
    simulator = Simulator(ui=True)
    # simulator = Simulator(ui=False, _matrix=[[60, 1000, 40, 15, 10, 65, 2, 3, 0.33333, 20, 4, 70, 8, 77, 3],
    # [60, 1000, 40, 15, 10, 65, 3, 3, 0.33333, 20, 4, 70, 8, 77, 3]])
